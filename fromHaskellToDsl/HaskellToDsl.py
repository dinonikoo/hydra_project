...
{function_defs}"""
    ...
    def function_def(self, items):
        ...
        if inferred == "a":
            ...
            # [a] считаю совместимым с любым конкретным списочным типом, это параметр
            if (
                inferred == "[a]"
        ...
        function_def = (
            ...
            f"{'  ' + dsl_expr if not dsl_expr.startswith(' ') else dsl_expr}"  
        )
    ...
    def at(self, items):
        return f"Lists.at ({items[1]}) ({items[0]})"  

    def cons(self, items):
        return f"Lists.cons ({items[0]}) ({items[1]})"  
...
--Добавить сравнение equal.string - выполнено
--В гидре чаров нет - задание через код буквы!
--неравно это not equal =>  /= - выполнено
--Контроль того, что в char есть импорт - выполнено
--Добавлена поддержка импортов

--функции без введённых параметров - выполенно

++ - выполнено 
-поддержка констант/определение - выполнено
...
--вставить классы - выполнено

--добавить сохранение имён функций в set, потоум что 2 с одним названием нельзя   - выполнено
